...
const broken=context.dataIntegrityReport();assert(broken.issues.some(x=>x.message.includes('الكمية في المخزن غير صالحة')),'negative stock detected');assert(broken.issues.some(x=>x.message.includes('الإجمالي الكلي مش متطابق')),'order total mismatch detected');assert(broken.issues.some(x=>x.message.includes('العربون غير منطقية')),'invalid deposit detected');assert(broken.issues.every(x=>typeof x.key==='string'&&x.key.length>0),'every issue has a stable key');
